...
class SPEncoder(nn.Module):
    ...
    def __init__(self, input_dims, embed_dim, num_heads, layers, attn_dropout=0.0, relu_dropout=0.0, 
                 ...
                 shift_mode=dict(I=['S','P','R'],X=['S','P','R'],S=['S','P','R'],C=[1,0.25,0.05]),
    ...
    def forward(self, a, t, v): # [BS, SL, D]
        ...
        else: a,t,v=a.permute(1,0,2),t.permute(1,0,2),v.permute(1,0,2)          # a shape = (360 250 81)
        a,t,v=self.init_emb(a),self.init_emb(t),self.init_emb(v)
        h_a,h_t,h_v=self.init_emb(h_a),self.init_emb(h_t),self.init_emb(h_v)    # h_a h_t h_v shape = (72,250,32)
        ...
        h_a = self.layer_norm(h_a)  #shape = (72,250,32)
...
class SPF(nn.Module): # Sparse Phased Fast attention
    def __init__(self,embed_dim,num_heads,attn_dropout,res_dropout,input_dim=None,stride=1,pf=None,
        ...
        self.use_fast=use_fast
        self.use_dense=use_dense
        self.reduce_dim=nn.AdaptiveAvgPool1d(output_size=8) #对采样出来的数据进行最高维度的归约 使其成为1
    ...
    def forward(self,x,x_k=None,x_v=None,shift=0,reverse=False):
        ...
        if self.use_dense:
            ...
            fetch=sparsify(x, context, self.stride, self.pf, shift, self.device)    #fetch shape (17,72,250,81)
            # print("fetch.size()",fetch.size())
            ...
            # print("c.size() original",c.size())
            # c=torch.transpose(c,0,-1)
           
            # print("c.size()  reshape once",c.size())
            # c=self.reduce_dim(c)
            # c=torch.transpose(c,0,-1)
            # print("c.size()  reshape twice",c.size())
...
def sparsify(hidden,context,stride,pf,shift=0,device='cuda'):
    ...
    r=(torch.arange(h).to(device))*stride+shift
