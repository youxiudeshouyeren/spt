    ...
    def evaluate(model, criterion, test=False,onlypunch=False):
        ...
        with torch.no_grad():
            for i_batch, (x_c, x_p, eval_attr) in enumerate(loader):
                ...
                #x1 = torch.randn(250, 360, 300)
                #x2 = torch.randn(250, 360, 81)
                #x3 = torch.randn(250, 360, 75)
                #modelData = "./pre_trained_models/demo.pt"
                #torch.onnx.export(net, x1, x2, x3, modelData)
