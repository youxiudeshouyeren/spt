...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader,test_only=False,verbose=True,ratio=1):
    ...
    def evaluate(model, criterion, test=False):
        ...
        with torch.no_grad():
            for i_batch, (batch_X, batch_Y, batch_META) in enumerate(loader):
                ...
                batch_size = text.size(0)
                # s_model=torch.jit.trace(model,example_inputs=(text, audio, vision),check_trace=False)
                # torch.jit.save(s_model, "resnet18.pt")


                # torch.onnx.export(model, (text, audio, vision), "1.onnx", 
                #                       input_names=['text', 'audio', 'vision'], output_names=['pred','hidden'],
    ...
    #torch.save(model, './gen_model/SPT.pth')
